...
특정 학교에 종속되지 않은 **일반 배포용**입니다. 학교 전용 자료실 링크·서식·소속 정보는 포함되어
있지 않으며, 자료실 탭에는 공개된 공식 링크(서울대 아로리, 교육부 STAR 포털)만 제공됩니다.
필요하면 `index.html`의 `LIBRARY_FILES` 배열에 직접 자료 링크를 추가할 수 있습니다.
